...
- `pooling_mode`: `FORBIDDEN` forbids simultaneous onboard sharing; `OPT_IN` requires every shared passenger to opt in; `ALLOWED` is the explicit laboratory evaluation default. Collective transport does not invent another jurisdiction's mixing rule.
...
- Time accounting distinguishes onboard `ride_duration` (pickup departure → dropoff arrival), `quota_debit` (policy basis) and `billable_service` (boarding + ride + alighting). Quota checks use the debit basis, not a cached ride summary.
...
Solvers, online insertion, disruption recovery and the independent notary read the same profile. Regulatory interpretation and commercial/pilot outcomes require separate evidence.

Regression: `python -m unittest tests.test_operator_policy`. Native-backed shadow/recovery methods skip when `mobiroute_native` is absent.
